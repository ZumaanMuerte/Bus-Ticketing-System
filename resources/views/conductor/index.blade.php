            ...
            <table class="w-full text-left border-t border-gray-200">
                ...
                <tbody>
                    @foreach($buses as $bus)
                        <tr>
                            <td class="p-2">{{ $bus->number }}</td>
                            <td class="p-2">{{ $bus->type }}</td>
                            <td class="p-2">{{ $bus->current_location }}</td>
                            <td class="p-2">{{ $bus->last_accessed_at ?? '—' }}</td>
                            <td class="p-2">
                                <!-- Modal Trigger -->
                                <button onclick="document.getElementById('modal-{{ $bus->id }}').classList.remove('hidden')" class="bg-blue-500 text-white px-3 py-1 rounded hover:bg-blue-600">Edit</button>
